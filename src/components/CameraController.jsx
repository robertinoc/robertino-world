...
export default function CameraController({ target, isMobile }) {
  ...
  useEffect(() => {
    ...
    camera.position.set(0, isMobile ? 1.4 : 1.5, z)
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (isZooming.current && target) {
      ...
      // Mobile: gentle drift — 2×2 grid is centered at y≈1.4
      // Desktop: wider horizontal sweep
      const driftX = isMobile ? Math.sin(t * 0.07) * 0.4 : Math.sin(t * 0.07) * 2.2
      const driftY = isMobile ? 1.4 + Math.cos(t * 0.05) * 0.12 : Math.cos(t * 0.05) * 0.85
      ...
      camera.lookAt(0, isMobile ? 1.4 : 0.2, 0)
    }
  })
